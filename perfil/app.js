...
document.addEventListener("DOMContentLoaded", function(){
    let user = db[Math.floor(Math.random() * db.length)];
    setUser(user);
    setPosts(user);
...
function setUser(user){
    ...
    document.getElementById("followers").innerHTML += Math.floor(Math.random()*1000);
    document.getElementById("follows").innerHTML += Math.floor(Math.random()*1000);
}
...
function getPost(i){
    return `<div class="p-0 col-12 col-md-6 col-lg-4 mx-auto p-1 pub-container rounded"><img src="${i<pubLink.length?pubLink[i]:pubLink[Math.floor(Math.random() * pubLink.length)]}" class="img-pub d-block mx-auto img-fluid rounded" /><div class="details text-light mx-auto text-center"><i class="fas fa-heart mx-2"></i>${Math.floor(Math.random()*120)}</div></div>`
}
